"""Building a Trackblazer race schedule: sharing, scarcity, and what it refuses.
...
Three of these exist because the bug happened:
...
def main():
  ...
  if E.load().get("Lady", {}).get("condition"):
    ...
    lady, stunning = turns("Lady"), turns("Stunning")
    ok("both want exactly three turns", len(lady) == 3 and len(stunning) == 3,
       (len(lady), len(stunning)))
    ok("and they are the same three turns", lady == stunning, sorted(map(str, lady ^ stunning)))
