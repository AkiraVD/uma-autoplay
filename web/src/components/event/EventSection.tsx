...
export default function EventSection({ config, updateConfig }: Props) {
  ...
  // Titles from the game's master.mdb, outcomes from data/events (server/master_data.py).
  // Throws on failure so the query retries instead of keeping an empty answer.
  // A server older than /data/events answers with the page, which isn't JSON.
  const getEventData = async () => {
    const res = await fetch(`${URL}/data/events`);
  ...
  return (
    <div className="bg-card p-6 rounded-xl shadow-lg border border-border/20">
      <h2 className="text-3xl font-semibold mb-6 flex items-center gap-3">
        <TicketsIcon className="text-primary" /> Events
      </h2>
      <IsOptimalEvent
