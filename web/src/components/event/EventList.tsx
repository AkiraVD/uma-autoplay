...
export default function EventList({ data, groupedChoices, eventChoicesConfig, addEventList, deleteEventList }: Props) {
  ...
  // With no filter picked, the search box looks through every event.
  const eventSelected = selected ? groupedChoices?.filter((val) => val.character_name.toLowerCase().includes(selected.toLowerCase())) : groupedChoices ?? [];

  return (
    <div>
      <Dialog>
        <DialogTrigger asChild>
          <Button className="w-full flex items-center gap-2 shadow-sm hover:shadow-md transition">
