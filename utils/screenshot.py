...
def enhanced_screenshot(region=(0, 0, 1920, 1080)) -> Image.Image:
  ...
  return enhance_for_reading(pil_img)

def enhance_for_reading(pil_img: Image.Image) -> Image.Image:
  """The preprocessing enhanced_screenshot applies, on an image you already have.

  Split out so a saved frame can be put through the same pipeline as a live
  grab, which is what makes the readers testable without the game running.
  """
